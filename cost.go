...
// CompilationError represents an expression that contained a compile-time
// error.
...
func (c *CompilationError) Error() string {
	return fmt.Sprintf("expression at %q has compilation error: %s", c.Path, c.Message)
}
...
// The last set of errors returned is for internal errors returned by
// cel.Compile.
func CheckExprCost(schema *structuralschema.Structural) ([]*CostError, []*CompilationError, []error) {
	// TODO(DangerOnTheRanger): swap this path system for field.Path
	return checkExprCost(schema, "<root>", rootCostInfo())
}

func checkExprCost(schema *structuralschema.Structural, path string, nodeCostInfo costInfo) ([]*CostError, []*CompilationError, []error) {
	results, err := schemacel.Compile(schema, false, schemacel.PerCallLimit)
	if err != nil {
		return nil, nil, []error{err}
	}
	var costErrors []*CostError
	var compileErrors []*CompilationError
	for _, result := range results {
		exprCost := getExpressionCost(result, nodeCostInfo)
		if result.Error != nil {
			// TODO(DangerOnTheRanger): fix inaccurate path as a part of PR #6
			compileErrors = append(compileErrors, &CompilationError{
				Path:    path,
	...
	switch schema.Type {
	case "array":
		itemCostErrors, itemCompileErrors, itemOtherErrors := checkExprCost(schema.Items, path+".<items>", nodeCostInfo.MultiplyByElementCost(schema))
		compileErrors = append(compileErrors, itemCompileErrors...)
		costErrors = append(costErrors, itemCostErrors...)
		otherErrors = append(otherErrors, itemOtherErrors...)
	case "object":
		var propCompileErrors []*CompilationError
		var propCostErrors []*CostError
		var propOtherErrors []error
		for propName, propSchema := range schema.Properties {
			propCostErrors, propCompileErrors, propOtherErrors = checkExprCost(&propSchema, path+"."+propName, nodeCostInfo.MultiplyByElementCost(schema))
			compileErrors = append(compileErrors, propCompileErrors...)
			costErrors = append(costErrors, propCostErrors...)
			otherErrors = append(otherErrors, propOtherErrors...)
		}
		if schema.AdditionalProperties != nil && schema.AdditionalProperties.Structural != nil {
			propCostErrors, propCompileErrors, propOtherErrors = checkExprCost(schema.AdditionalProperties.Structural, path+"."+"<properties>", nodeCostInfo.MultiplyByElementCost(schema))
			compileErrors = append(compileErrors, propCompileErrors...)
			costErrors = append(costErrors, propCostErrors...)
			otherErrors = append(otherErrors, propOtherErrors...)
		}
	}
	return costErrors, compileErrors, otherErrors
}
